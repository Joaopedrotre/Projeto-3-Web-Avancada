...
import type { Categoria, Produto, RespostaApi } from './types.js';
import { atualizarProduto, criarProduto, listarProdutos, removerProduto } from './api.js';

const formatadorMoeda = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' });

let categoriasDisponiveis: Categoria[] = [];
let produtoEmEdicaoId: number | null = null;
...
async function carregarProdutos(): Promise<void> {
    ...
    try {
        const resposta: RespostaApi<Produto[]> = await listarProdutos(termoBusca);
...
function registrarEventosLinhas(produtos: Produto[]): void {
    document.querySelectorAll<HTMLButtonElement>('.btn-editar').forEach((botao) => {
    ...
    document.querySelectorAll<HTMLButtonElement>('.btn-remover').forEach((botao) => {
        botao.addEventListener('click', () => {
            const id = Number(botao.dataset.id);
...
function abrirModalEdicao(produto: Produto): void {
...
function definirTituloModal(titulo: string): void {
...
function abrirModalBootstrap(): void {
    ...
    const bootstrapGlobal = (window as unknown as { bootstrap: { Modal: new (el: Element) => { show: () => void } } }).bootstrap;
...
function preencherFormulario(produto: Produto): void {
...
function obterCampo(id: string): HTMLInputElement {
...
function fecharModalBootstrap(): void {
    const elementoModal = document.getElementById('modal-produto');
    if (elementoModal === null) {
        return;
    }
    const bootstrapGlobal = (window as unknown as { bootstrap: { Modal: { getInstance: (el: Element) => { hide: () => void } | null } } }).bootstrap;
...
// ---------------------------------------------------------------------
function confirmarRemocao(id: number, nome: string): void {
...
async function removerProdutoEAtualizar(id: number): Promise<void> {
    try {
...
// ---------------------------------------------------------------------
function exibirNotificacao(mensagem: string, tipo: 'success' | 'danger'): void {
    ...
    container.insertAdjacentHTML(
        'beforeend',
        `<div id="${idToast}" class="toast align-items-center ${corFundo} border-0" role="alert">
            <div class="d-flex">
    ...
    const bootstrapGlobal = (window as unknown as { bootstrap: { Toast: new (el: Element) => { show: () => void } } }).bootstrap;
...
// ---------------------------------------------------------------------
export async function inicializarTelaProdutos(): Promise<void> {
...
export function definirCategorias(categorias: Categoria[]): void {
